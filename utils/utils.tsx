...
export async function fetchWithTimeout(url: string, options = {}, timeout = 3000) {
...
export const getSystemPrompt = (finalResults: { fullContent: string }[] | undefined, ageGroup: string) => {
  const results = finalResults && Array.isArray(finalResults) ? finalResults : []
  ...
  <teaching_info>
  ${"\n"}
  ${results.slice(0, 7).map((result, index) => `## Webpage #${index}:\n ${result.fullContent} \n\n`)}
  </teaching_info>
